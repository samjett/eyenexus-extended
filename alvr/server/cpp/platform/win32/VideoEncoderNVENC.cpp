...
void VideoEncoderNVENC::FillEncodeConfig(NV_ENC_INITIALIZE_PARAMS &initializeParams, int refreshRate, int renderWidth, int renderHeight, uint64_t bitrate_bps)
{
	...
	// We use CONSTQP to set a baseline quality, and then provide a Delta Map (qpMapMode = NV_ENC_QP_MAP_DELTA)
	// to apply Gaussian-based degradation to the periphery.
	encodeConfig.rcParams.qpMapMode = NV_ENC_QP_MAP_DELTA;
	
	// Disable internal Adaptive Quantization (AQ) to ensure our custom QP map is the sole driver of spatial quality.
	...
	// Set the Base QP. Foveal region will be close to this value.
	// Peripheral regions will have positive offsets added to this (higher QP = lower quality).
