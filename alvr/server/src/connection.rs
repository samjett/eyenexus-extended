...
// EyeNexus-Gaze Mapping: VR to Screen Space Coordinates.
/// Computes the screen-space coordinates (X, Y) of the eye gaze based on FOV and gaze angles.
///
/// # Arguments
/// * `frame_width` - Width of a single eye's view in pixels
/// * `frame_height` - Height of a single eye's view in pixels
/// * `yaw` - Gaze yaw angle in radians
/// * `pitch` - Gaze pitch angle in radians
/// * `*_fov_*` - Field of View angles for left/right eyes
///
/// # Returns
/// A tuple (left_x, left_y, right_x, right_y) representing the pixel coordinates.
/// Note: Y coordinates are calculated from the bottom up (0 = bottom).
            ...
            while is_streaming(&client_hostname) {
                ...
                if let Some(sink) = &mut face_tracking_sink {
                    ...
                    if let [Some(left), Some(right)] = face_data.eye_gazes {
                        ...
                        // Compute gaze location (X_o, Y_o) for both eye from VR to Screen Space Coordinates
                        let (left_frame_x,left_frame_y,right_frame_x,right_frame_y)= compute_eye_gaze_location(frame_width,frame_height,left_yaw as f64, 
                            left_pitch as f64, tracking.left_view_fov.left as f64, tracking.left_view_fov.right as f64, tracking.left_view_fov.up as f64, 
                            tracking.left_view_fov.down as f64, tracking.right_view_fov.left as f64, tracking.right_view_fov.right as f64, tracking.right_view_fov.up as f64, 
                            tracking.right_view_fov.down as f64);
                        // Update gaze location (X_o, Y_o) for both eye in Screen Space Coordinates
                    ...
                    sink.send_tracking(face_data);// update gaze location (X_o, Y_o) for both eye in Screen Space Coordinates to OSC server
                }
